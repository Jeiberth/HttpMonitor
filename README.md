...
```bash
HttpMonitor.exe [/L:<log_file>] <duration_in_seconds>
...
| --- | --- |
| **Tracking requests** | $O(1)$ average using `std::unordered_map` |
...
6. Configure the following paths in your Project Properties:
...
**Note:** Build the project using the **x64 configuration**.
...
Packet capture typically requires **administrator privileges**, so the program should be run from an elevated terminal (Run as Administrator).
